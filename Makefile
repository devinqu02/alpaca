...
RUNTIME_PASSES = -load $(BIN_DIR)/stats_pass.so -mem2reg -stats-pass -simplifycfg
TEST_PASSES = -load $(BIN_DIR)/alpaca_pass.so -load $(BIN_DIR)/stats_pass.so -mem2reg -alpaca-pass -stats-pass -simplifycfg
...
$(BIN_DIR)/alpaca_runtime.o: $(SRC_DIR)/alpaca_runtime.c $(BIN_DIR)/stats_pass.so
	...
	opt -enable-new-pm=0 $(RUNTIME_PASSES) $(BIN_DIR)/alpaca_runtime.bc | llc -filetype=obj -o $@
...
$(BIN_DIR)/%.o: $(BIN_DIR)/%.bc $(BIN_DIR)/alpaca_pass.so $(BIN_DIR)/stats_pass.so
	opt -enable-new-pm=0 $(TEST_PASSES) $(BIN_DIR)/$*.bc -o $(BIN_DIR)/$*.bc
	llvm-dis $(BIN_DIR)/$*.bc
...
# No Alpaca (link with empty runtime, run stats-pass)
$(BIN_DIR)/alpaca_empty_runtime.o: $(SRC_DIR)/alpaca_empty_runtime.c $(BIN_DIR)/stats_pass.so
	$(CC) $(CFLAGS) $(CPPFLAGS) -O0 -Xclang -disable-O0-optnone -fno-discard-value-names -emit-llvm -S $< -o $(BIN_DIR)/alpaca_runtime.bc
	opt -enable-new-pm=0 $(RUNTIME_PASSES) $(BIN_DIR)/alpaca_runtime.bc | llc -filetype=obj -o $@

$(BIN_DIR)/benchmarks_%_no_alpaca.out: $(BENCHMARKS_DIR)/%.c $(BIN_DIR)/stats_pass.so $(BIN_DIR)/alpaca_empty_runtime.o
	$(CC) $(CFLAGS) $(CPPFLAGS) -O0 -Xclang -disable-O0-optnone -fno-discard-value-names -emit-llvm -S $< -o $(BIN_DIR)/benchmarks_$*_no_alpaca.bc
	opt -enable-new-pm=0 $(TEST_PASSES) $(BIN_DIR)/benchmarks_$*_no_alpaca.bc | llc -filetype=obj -o $(BIN_DIR)/benchmarks_$*_no_alpaca.o
	$(CC) $(CFLAGS) $(CPPFLAGS) -o $@ -fno-pie -no-pie $(BIN_DIR)/benchmarks_$*_no_alpaca.o $(BIN_DIR)/alpaca_empty_runtime.o
