...
void task_transform() {
    ...
    for (i = 0; i < ACCEL_WINDOW_SIZE; i++) {
        if (GV(window, i).x < SAMPLE_NOISE_FLOOR ||
            ...
            GV(window, i).x =
                (GV(window, i).x > SAMPLE_NOISE_FLOOR) ? GV(window, i).x : 0;
            GV(window, i).y =
                (GV(window, i).y > SAMPLE_NOISE_FLOOR) ? GV(window, i).y : 0;
            GV(window, i).z =
                (GV(window, i).z > SAMPLE_NOISE_FLOOR) ? GV(window, i).z : 0;
        }
    }
...
void task_classify() {
    ...
    LOG("classify: mean: %ld\r\n", meanmag);
    LOG("classify: stddev: %ld\r\n", stddevmag);
    ...
    for (i = 0; i < MODEL_SIZE; ++i) {
        ...
        LOG("classify: stat_mean_err: %ld\r\n", stat_mean_err);
        LOG("classify: stat_stddev_err: %ld\r\n", stat_sd_err);
        ...
        LOG("classify: move_mean_err: %ld\r\n", move_mean_err);
        LOG("classify: move_stddev_err: %ld\r\n", move_sd_err);
...
void task_stats() {
    ...
    if (GV(totalCount) == SAMPLES_TO_COLLECT) {
        unsigned resultStationaryPct =
            GV(stationaryCount) * 100 / GV(totalCount);
        ...
        PRINTF("stats: s %u (%u%%) m %u (%u%%) sum/tot %u/%u: %c\r\n",
               GV(stationaryCount), resultStationaryPct, GV(movingCount),
               resultMovingPct, GV(totalCount), sum,
               sum == GV(totalCount) ? 'V' : 'X');
        TRANSITION_TO(task_idle);
    } else {
        TRANSITION_TO(task_sample);
    }
