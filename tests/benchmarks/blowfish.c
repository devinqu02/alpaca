...
static const char cp[32] = {'1', '2', '3', '4', '5', '6', '7', '8', '9',
                            '0', 'A', 'B', 'C', 'D', 'E', 'F', 'F', 'E',
                            'D', 'C', 'B', 'A', '0', '9', '8', '7', '6',
                            '5', '4', '3', '2', '1'};  // mimicing 16byte hex
                                                       // key
                                                       // (0x1234_5678_90ab_cdef_fedc_ba09_8765_4321)
static const char indata[LENGTH] = {'H', 'e', 'l', 'l', 'o', ',', ' ',
                                    'w', 'o', 'r', 'l', 'd', '!'};
    ...
    while (i < 32) {
        if (cp[i] >= '0' && cp[i] <= '9')
            by = (by << 4) + cp[i] - '0';
                 ...
                 cp[i] <= 'F')  // currently, key should be 0-9 or A-F
            by = (by << 4) + cp[i] - 'A' + 10;
        else
...
#if VERBOSE > 0
                if (GV(index2) == 256 * 2 + 20 ||
                    GV(index2) == (256 * 3 - 2) + 20) {
...
#if VERBOSE > 0
                if (GV(index2) == 256 * 3 + 20 ||
                    GV(index2) == (256 * 4 - 2) + 20) {
