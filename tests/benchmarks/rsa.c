...
void task_init() {
    ...
    unsigned message_length =
        sizeof(PLAINTEXT) - 1;  // skip the terminating null byte
...
void task_pad() {
    ...
    for (i = 0; i < NUM_PAD_DIGITS; ++i) LOG("%x ", PAD_DIGITS[i]);
    LOG("'");
    ...
    GV(block, 0) = 1;
    for (i = 1; i < NUM_DIGITS; ++i) GV(block, i) = 0;
...
void task_mult_block_get_result() {
    ...
    if (GV(exponent) > 0) {
        // TODO: current implementation restricts us to send only to the next
        // instantiation of self, so for now, as a workaround, we proxy the
        // value in every instantiation
...
void task_reduce_normalizable() {
    ...
    for (i = GV(reduce); i >= 0; --i) {
        LOG("normalizable: m[%u]=%x n[%u]=%x\r\n", i, GV(product, i),
            i - GV(offset), GV(modulus, i - _global_offset));
...
void task_reduce_normalize() {
    ...
    if (GV(offset) >
        0) {  // l-1 > k-1 (loop bounds), where offset=l-k, where l=|m|,k=|n|
...
void task_reduce_n_divisor() {
    ...
    // Divisor, derived from modulus, for refining quotient guess into exact
    // value
    GV(n_div) = (GV(modulus, NUM_DIGITS - 1) << DIGIT_BITS) +
                GV(modulus, NUM_DIGITS - 2);
...
void task_reduce_quotient() {
    ...
    n_q = ((uint32_t)GV(product, _global_reduce) << (2 * DIGIT_BITS)) +
          (GV(product, _global_reduce - 1) << DIGIT_BITS) +
          GV(product, _global_reduce - 2);

    LOG("reduce: quotient: m[d]=%x m[d-1]=%x m[d-2]=%x n_q=%x%x\r\n",
        GV(product, _global_reduce), GV(product, _global_reduce - 1),
        GV(product, _global_reduce - 2), (uint16_t)((n_q >> 16) & 0xffff),
        (uint16_t)(n_q & 0xffff));
...
void task_reduce_multiply() {
    ...
    for (i = offset; i < 2 * NUM_DIGITS; ++i) {
        ...
        // TODO: consider adding number of digits to go along with the 'product'
        // field, then we would not have to zero out the MSDs
        m = c;
...
void task_reduce_subtract() {
    ...
    // The qn product had been shifted by this offset, no need to subtract the
    // zeros
    offset = GV(reduce) + 1 - NUM_DIGITS;
    ...
    for (i = 0; i < 2 * NUM_DIGITS; ++i) {
        ...
        // For calling the print task we need to proxy to it values that we do
        // not modify
        if (i >= offset) {
            qn = GV(product2, i);
    ...
    } else {  // reduction finished: exit from the reduce hypertask (after
              // print)
