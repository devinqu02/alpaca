...
filtered_versions = [ "privatize_all", "war_only", "alpaca"]
colors = {
    "privatize_all": ("#32CD32", "#228B22"),
    "war_only": ("#87CEEB", "#1E3A8A"),
    "alpaca": ("#FFD700", "#FF8C00"),
}
...
for program in df["Program"].unique():
    ...
    loads_value = df[(df["Program"] == program) & (df["Version"] == "privatize_all")]["Loads"].values
    stores_value = df[(df["Program"] == program) & (df["Version"] == "privatize_all")]["Stores"].values
    ...
    bytes_loaded_value = df[(df["Program"] == program) & (df["Version"] == "privatize_all")]["Bytes Loaded"].values
    bytes_stored_value = df[(df["Program"] == program) & (df["Version"] == "privatize_all")]["Bytes Stored"].values
    ...
    bytes_used_value = df[(df["Program"] == program) & (df["Version"] == "privatize_all")]["Bytes Used"].values
...
ax.set_title("Memory Accessed by Program and Version")
ax.set_xticks(x + bar_width)
ax.set_xticklabels(programs, rotation=45)
ax.legend(title="Version")
plt.tight_layout()
plt.savefig("graphs/bytes_loads_stores_stacked.png")
plt.show()
...
ax.set_title("# of Memory Accesses by Program and Version")
ax.set_xticks(x + bar_width)
ax.set_xticklabels(programs, rotation=45)
ax.legend(title="Version")
plt.tight_layout()
plt.savefig("graphs/counts_loads_stores_stacked.png")
plt.show()
...
ax.set_title("Memory Used by Program and Version")
ax.set_xticks(x + bar_width)
ax.set_xticklabels(programs, rotation=45)
ax.legend(title="Version")
plt.tight_layout()
plt.savefig("graphs/memory_used.png")
plt.show()
